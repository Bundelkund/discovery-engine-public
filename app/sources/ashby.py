            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                for slug in slugs:
                    try:
                        ...
                        logger.warning(f"Ashby slug '{slug}' failed: {e}")
                        continue

            logger.info(
                f"Ashby: fetched {len(all_jobs)} jobs from {len(slugs)} boards"
            )
            return all_jobs
        except Exception as e:
            logger.error(f"Ashby fetch failed: {e}")
            return []

    def _load_slugs(self, portals_path: Path) -> list[str]:
        """Extract Ashby slugs from portals.yaml."""
        ...
        for company in data.get("tracked_companies", []):
            ...
            if "ashbyhq.com" in careers_url:
                parts = careers_url.split("jobs.ashbyhq.com/")
                if len(parts) > 1:
