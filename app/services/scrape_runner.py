...
async def run_due(min_interval_hours: int = 24) -> dict:
    ...
    'done' (with stats) or, on error, 'failed'. ONLY a 'done' run resets the
    cadence clock, so a failed source retries next tick. Per-source isolation: one
    source raising never stops the others. Single-flight: a no-op returning
    ...
    try:
        ...
        for source_id in _enabled_sources():
            ...
            try:
                resp = await ScrapeOrchestrator(client).run(source_id=source_id)
                stats = {
...
async def scheduler_loop(
    ...
    while not stop.is_set():
        try:
            await run_due(min_interval_hours=min_interval_hours)
        except Exception as exc:  # noqa: BLE001
