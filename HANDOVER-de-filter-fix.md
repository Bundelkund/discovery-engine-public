...
> - **Slice 4 (P5 null-reclassify) AUSGEFÜHRT via Feed-Re-Scan** (nicht SQL-fabriziert): keine DE-Signale gespeichert → 3019 null-Feeds neu gescannt via `export_null_slugs.py` → `ats_scanner --slugs-file` → `seed`. Ergebnis: **+142 Firmen wieder monitored** (140 de + 2 remote), null 3019→2499 (−520), foreign +378. Recruitee trug es (135 de von 809). **factorial:** Sitemap trägt keine Location → Feed-Re-Scan ließ 140 aktive null. Nachgelagert via `scripts/recover_factorial_de.py` (Job-URL-Slug + Job-Seiten-📍-Location → `_cat_str`/`_fold`): 140 aktive aufgelöst → 4 de + 3 remote (7 monitored) + 133 foreign; 82 dead bleiben null. factorial null 222→82. Bestätigt: `.com`-Tenants stark spanisch. Restliche Gesamt-null = jobless/dead Feeds. Loops = wiederholbare operative Tasks (`export_null_slugs.py`, `recover_factorial_de.py`).
