...
- **ATS registry chain (CC-enumeration → storage-gate → db-driven-slugs → softgarden)** — `ats_companies` is now the slug source-of-truth: Common-Crawl enumeration over 8 slug-ATS (~8.4k raw → 6013 active feeds, see [docs/ats-enumerability.md](docs/ats-enumerability.md)). ① **storage-gate** — title-level filter drops flood/negative titles before description-resolution, tags primary-role hits `priority` via `dq_flags` (keeps `jobs` clean at `store_threshold=0`). ② **db-driven-slugs** — all 7 slug-scrapers union active DB slugs (`status=active AND monitor`) with the `portals.yaml` fallback via `merge_slugs`. ③ **softgarden** — new provider (CC-verified, `career.softgarden.de` → `/jobs.json` schema.org DataFeed) + `SoftgardenScraper` (`/scrape/softgarden`, runtime-validated) + 104 boards seeded (98 active). *(6106f81, ae0ce77, 6362ca6)*
