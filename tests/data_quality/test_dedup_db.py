...
def test_restart_sim_near_duplicate_persists():
    ...
    client_b = MagicMock()
    # Simulate DB having one of the band rows stored
    client_b.table.return_value.select.return_value.in_.return_value.limit.return_value.execute.return_value.data = [
        {"band_hash": bands[0]}
    ]
